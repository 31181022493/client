...
const listProducts =(pagenumber) => async (dispatch) =>{
    try{
        ...
        const {data} = await axios.get("https://cloudnhom2.herokuapp.com/api/Products/?pageIndex="+ pagenumber);
...
const detailsProduct = (productId) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get('https://cloudnhom2.herokuapp.com/api/Products/'+ productId);
...
const productcount = () => async(dispatch) =>{
  try{
    ...
    const{count} =await axios.get('https://cloudnhom2.herokuapp.com/api/Products/');
...
const searchproduct =(brand) => async(dispatch)=>{
  try{
    ...
    const {data} = await axios.get("https://cloudnhom2.herokuapp.com/api/Products?Search="+ brand);
