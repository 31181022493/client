...
dispatch({type: USER_SIGNIN_REQUEST, payload:{email, password}});
    try{
        const {data} = await axios.post( "https://cloudnhom2.herokuapp.com/api/Account/login",{email, password})
...
const register = (displayName, email, password) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post('https://cloudnhom2.herokuapp.com/api/Account/register', {
      displayName,
      email,
      password,
    });
